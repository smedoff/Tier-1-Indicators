---
title: "Comparing 05a - Deep 7 CML tables"
...
year <- 2022
discrete_variables <- c("VESSEL", "VESSELS", "CML")
...
last_year.df <- read.csv(file.path("..",
                                  ...
                                  year-1,
                                  "final",
                                  paste0("TIER_1_DP7_CML_RPT_", year-1, ".csv")))


this_year.df <- read.csv(file.path("..",
                                  "final",
                                  paste0("TIER_1_DP7_CML_RPT_", year, ".csv")))


```

```{r}
different_obs.df <- anti_join(this_year.df, last_year.df)

this_cml <- this_year.df %>% 
  filter(SEASON %in% different_obs.df$SEASON, 
         CML %in% different_obs.df$CML)

last_cml <- last_year.df %>% 
  filter(SEASON %in% different_obs.df$SEASON, 
         CML %in% different_obs.df$CML)

differences_in_obs.df <- left_join(this_cml, last_cml, by = c("SEASON", "CML"))

names(differences_in_obs.df) <- gsub(".y", ".2021", gsub(".x", ".2022", names(differences_in_obs.df)))

write_csv(differences_in_obs.df, file.path("Additional Observations", "D7_CML_differences.csv"))
196 variab```
...
continuous_variables <- setdiff(common_variables, c(discrete_variables, "SEASON"))
